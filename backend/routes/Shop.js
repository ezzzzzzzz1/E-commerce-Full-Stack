...
router.post(
    ...
    async (req, res) => {
      try {
        ...
        await query("insert into shop set ? ", product);
        res.status(200).json({
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const product = await query("select * from shop where id = ?", [
        req.params.id,
      ]);
      if (!product[0]) {
      ...
      // 4- UPDATE product
      await query("update shop set ? where id = ?", [productObj, product[0].id]);
...
router.delete(
    ...
    async (req, res) => {
      try {
        ...
        await query("delete from shop where id = ?", [product[0].id]);
...
router.get("",async (req,res)=>{
  ...
  const products = await query(`select * from shop ${search}`)
